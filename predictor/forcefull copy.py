...
class EfficientPred:
    def __init__(self, LLMInference, cal_dataset, pred_dataset, partial, batch_size, alpha):
        ...
        self.cal_total_batch = math.ceil(len(self.cal_sub)/self.batch_size)
    ...
    def calibrate(self):
        ks = []
        with open(f"outputs_exp/{self.LLMInference.name}_{self.cal_dataset.name}_ppl.pkl", "rb") as f:
            cal_ppl = pickle.load(f)
        with open(f"outputs_exp/{self.LLMInference.name}_{self.cal_dataset.name}_min_tokens_fixed.jsonl", "r", encoding="utf-8") as f:
            lines = f.readlines()
            for l in lines:
                line = json.loads(l)
                q = line["question"]
                if line["flag"] == "Correct":
                    ...
                    token_used = line['token_used']
                    ppls = cal_ppl[q]['ppl']
                    k = (ppls[resp_start+token_used] - ppls[resp_start+token_used-100])/100
                    ks.append(k)
        n = len(ks)
    ...
    def predict(self):
        ...
        with open(f"outputs_exp/{self.LLMInference.name}_{self.pred_dataset.name}_ppl.pkl", "rb") as f:
            pred_ppl = pickle.load(f)
        budgets = []
        for q in tqdm(questions, desc="Predicting token budget:"):
            resp_start = pred_ppl[q]['resp_start']
            ppls = pred_ppl[q]['ppl']
            ks = [(ppls[resp_start+i*100] - ppls[resp_start+i*100-100])/100 for i in range(1, int((len(ppls)-resp_start)/100))]
            budget = None
        ...
        final_answers = self.final_answer(query_token_ids)
        with open(f"result/Calibrate_{self.LLMInference.name}_{self.pred_dataset.name}_final.jsonl", "w", encoding="utf-8") as f:
